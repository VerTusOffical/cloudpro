...
# Скрипт для исправления install.sh
...
# Убираем вывод об успешной установке в начале скрипта
sed -i '/^# Проверяем IP-адрес сервера$/,/^exit 0$/d' install.sh

# Добавляем в конец файла оригинальный код установки
cat >> install.sh << 'EOL'
...
echo
echo -e "База данных: ${GREEN}$PANEL_DB_NAME${NC}"
echo -e "Пользователь БД: ${GREEN}$PANEL_DB_USER${NC}"
echo -e "Пароль БД: ${GREEN}$PANEL_DB_PASS${NC}"
...
echo "Файл install.sh успешно исправлен!"
echo "Резервная копия сохранена в install.sh.bak" 